...
func rootNodes(ctx paletteCtx) []paletteNode {
	return []paletteNode{
		...
		{
			id:      string(commandPaletteCommandRecordVoiceNote),
			title:   "Record voice note",
			detail:  "Start recording a voice note and queue it when you stop.",
			meta:    "VOICE",
			aliases: []string{"record", "audio", "microphone"},
			visible: func(c paletteCtx) bool { return c.hasPeer && !c.voiceRecordingActive },
			action:  &commandPaletteAction{command: commandPaletteCommandRecordVoiceNote},
		},
		{
			id:      string(commandPaletteCommandStopRecording),
			title:   "Stop recording",
			detail:  "Stop the active recording and queue it for sending.",
			meta:    "VOICE",
			aliases: []string{"stop", "record", "audio"},
			visible: func(c paletteCtx) bool { return c.voiceRecordingActive },
			action:  &commandPaletteAction{command: commandPaletteCommandStopRecording},
		},
		{
			id:      string(commandPaletteCommandCancelRecording),
			title:   "Cancel recording",
			detail:  "Cancel the active voice note recording without queuing it.",
			meta:    "VOICE",
			aliases: []string{"cancel", "record", "audio"},
			visible: func(c paletteCtx) bool { return c.voiceRecordingActive },
			action:  &commandPaletteAction{command: commandPaletteCommandCancelRecording},
		},
		{
